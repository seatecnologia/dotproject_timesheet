...
if($do_report){
	...
	$sql = "SELECT u.user_id,u.user_username,contact_first_name,contact_last_name FROM users AS u LEFT JOIN contacts ON contact_id = user_contact";
	if ($log_userfilter!=0) {
			$sql.=" WHERE user_id=".$log_userfilter;
	...
	$sql = 	 "SELECT t.* FROM tasks AS t LEFT JOIN projects on project_id = task_project ";

	if ($use_period) {
		if (!$where) { $sql.=" WHERE ";$where=true; $and=true;}
		/*$sql.="( (task_start_date >= $ss AND task_start_date <= $se ) OR ( task_end_date <= $se AND task_end_date >= $ss ) ) ";*/
		$sql.="( (task_start_date >= $ss AND task_start_date <= $se ) AND ( task_end_date <= $se OR task_end_date >= $ss ) ) ";
		
		//$and=true;
	}
	      $sql.="AND !isnull(task_end_date) AND task_end_date != '0000-00-00 00:00:00'";
	      $sql.="AND !isnull(task_start_date) AND task_start_date != '0000-00-00 00:00:00'";
	      $sql.="AND task_dynamic   ='0'";
	      $sql.="AND task_milestone = '0'";
	      $sql.="AND task_duration  > 0";
			//;
